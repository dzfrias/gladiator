...
var level: int:
	...
	set(level):
		_level = level
		save()
var done_tutorial: bool:
	get: return _done_tutorial
	set(new):
		_done_tutorial = new
		save()
...
func load_data() -> void:
	...
	while file.get_position() < length:
		...
		if not json.data.has_all(["buckles", "alternate", "gadget", "passives", "level", "done_tutorial"]):
		...
		_done_tutorial = json.data["done_tutorial"]
